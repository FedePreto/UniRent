...
class Locatore{
    ...
    public function getContratto($id_richiesta){
        ...
        if($max_richieste >= $id_richiesta and $id_richiesta>0){
            $alloggio = Richieste::join('alloggi','richieste.id_alloggio','=','alloggi.id')->where('richieste.id','=',$id_richiesta)->select('alloggi.id','alloggi.indirizzo','alloggi.periodo_locazione','alloggi.prezzo','alloggi.tipologia','richieste.stato')->get();
